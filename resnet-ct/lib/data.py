...
def load_data(opt):
    ...
    if opt.dataset in ['cifar10']:
        ...
        drop_last_batch = {'train': True, 'test': False}
        shuffle = {'train': True, 'test': True}
        transform = transforms.Compose([transforms.Resize((opt.isize_y, opt.isize_x)),
                                        transforms.ToTensor(),
                                        transforms.Normalize((0.5, 0.5, 0.5), (0.5, 0.5, 0.5)), ])

        # dataset = {x: ImageFolder(os.path.join(opt.dataroot, x), transform) for x in splits}
        dataset = {x: LungDataset(dataroot=os.path.join(opt.dataroot, x), transform=transform) for x in splits}
...
class LungDataset(Dataset):
    def __init__(self, transform, dataroot):
        self.transform = transform
        self.dataroot = dataroot
        self.datalist = []
        for target in os.listdir(self.dataroot):
            for img in os.listdir(os.path.join(self.dataroot, target)):
                self.datalist.append([os.path.join(self.dataroot, target, img), np.array(int(target)), img])

    def __getitem__(self, idx):
        img, label, name = self.datalist[idx]
