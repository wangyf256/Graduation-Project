...
class ModifiedResNetdecoder(nn.Module):
    def __init__(self):
        ...
        self.decoder = nn.Sequential(BasicBlockdecoder(256, 128, True),
                                     BasicBlockdecoder(128, 128),
                                     BasicBlockdecoder(128, 64, True),
                                     BasicBlockdecoder(64, 64),
                                     BasicBlockdecoder(64, 64, True))
...
class ctResNet(nn.Module):
    ...
    def __init__(self):
        ...
        self.encoder = ModifiedResNet(block=BasicBlock, layers=[2, 2, 2, 2])  # resnet18
        # self.encoder = ModifiedResNet(block=BasicBlock, layers=[3, 4, 6, 3])  # resnet34
        self.decoder = ModifiedResNetdecoder()

    def forward(self, x):
        x1, x2, x3 = self.encoder(x)
        # print("x1,x2,x3 shape: ", x1.shape, x2.shape, x3.shape)  # [2, 64, 32, 32] [2, 128, 16, 16] [2, 256, 8, 8]
